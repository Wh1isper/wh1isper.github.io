...
本文对应中国大学mooc-人工智能实践：Tensorflow笔记第五讲内容，借由mnist数据集学习全连接网络基础。
...
保存当前图结构的.meta文件、保存当前参数名的.index文件、保存当前参数的.data文件。
	...
	with tf.Session() as sess:
		for i in range(STEPS):
			if i % 轮次 == 0:
				saver.save(sess, os.path.jion(MODEL_SAVE_PATH,MODEL_NAME),
